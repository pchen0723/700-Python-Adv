...
class Secret: 
    ...
    def __setattr__(self, attribute_name, value):
        if attribute_name == "secret":
            raise NameError ("You can't change the %s to %s" % (attribute_name, value))
            ...
            raise AttributeError ("%s is not an attribute for class %s" % (attribute_name, str(self.__class__).split('.')[1]))
